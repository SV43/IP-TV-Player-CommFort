(*
 *******************************************************************************
 * PasLibVlcUnit.pas - pascal interface for VideoLAN libvlc 3.0.1
 *
 ...
 *)
type
  libvlc_media_open_cb = function(opaque : Pointer; var datap : Pointer; var sizep : Int64) : Integer; cdecl;
 ...
 *)
type
  libvlc_media_read_cb = function(opaque : Pointer; buf : Pointer; len : libvlc_size_t) : libvlc_ssize_t; cdecl;
 ...
 *)
type
  libvlc_media_close_cb = procedure(opaque : Pointer);
...
type
  libvlc_video_lock_cb_t = function(opaque : Pointer; planes : PVCBPlanes) : Pointer; cdecl;
...
type
  libvlc_video_unlock_cb_t = procedure(opaque : Pointer; picture : Pointer; planes : PVCBPlanes); cdecl;
...
type
  libvlc_video_display_cb_t = procedure(opaque : Pointer; picture : Pointer); cdecl;
...
type
  libvlc_video_cleanup_cb_t = procedure(opaque : Pointer); cdecl;
...
var
  libvlc_handle : {$IFDEF FPC}dynlibs.TLibHandle{$ELSE}THandle{$ENDIF};
...
begin
  ...
  // function SetDefaultDllDirectories(_In_ DWORD DirectoryFlags) : BOOL; stdcall; external; kernel32.dll
  {$ENDIF};
...
{$IFDEF LINUX}
  ...
  begin
    libvlc_dynamic_dll_error := 'Library not found ' + LIBVLC_DLL_NAME + '!';
    exit;
  end;
{$ENDIF}
  ...
  // try load library
  // before loading libvlc.dll application must change dir
  // to libvlc_dynamic_dll_path
...
{$IFNDEF MSWINDOWS}
  if not libvlc_dll_get_proc_addr(@libvlc_media_player_set_evas_object,
    'libvlc_media_player_set_evas_object') then exit;
{$ENDIF}
