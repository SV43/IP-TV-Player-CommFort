...
begin
  ...
  try
    Ini.WriteString('Settings','VideoLan VLC Dll', Form1.dePachVLC.Text);
    Ini.WriteString('Settings','Style', Form1.lePachStyle.Text);
    Ini.WriteString('Settings','Chann IPTV Plug', Form1.cbIPTVchan.Text);
    Ini.WriteString('Settings','URL M3U', Form1.edURLM3U.Text);
    Ini.WriteString('Settings','URL JTV', Form1.edURLJTV.Text);
  finally
    Ini.Free;
  end;
end;
...
begin
  ...
  Ini := TIniFile.Create(path+'IPTV_Plugin\IPTV_Plug.ini');
  try
    Form1.dePachVLC.Text := Ini.ReadString('Settings', 'VideoLan VLC Dll', path + 'IPTV_Plugin\VLC');
    Form1.lePachStyle.Text := Ini.ReadString('Settings', 'Style', path + 'IPTV_Plugin\style\');
    Form1.cbIPTVchan.Text := Ini.ReadString('Settings', 'Chann IPTV Plug','IP-TV');
    Form1.edURLM3U.Text   := Ini.ReadString('Settings', 'URL M3U','');
    Form1.edURLJTV.Text   := Ini.ReadString('Settings', 'URL JTV','https://site.ru/jtv.zip2');
  finally
    Ini.Free;
  end;
end;
...
begin
  ...
  try
    ...
    ForceDirectories(Form1.lePachStyle.Text + '\logo-channels\');
...
begin
  ...
  Form1.cbIPTVchan.Items.BeginUpdate;
  try
    Form1.cbIPTVchan.Items.Clear;
    for c := 1 to g do
    begin
      ...
      if CommFortChanel <> '' then
        Form1.cbIPTVchan.Items.Add(CommFortChanel);
    end;

    for i := 0 to Form1.cbIPTVchan.Items.Count - 1 do
      if SameText(Form1.cbIPTVchan.Items[i], StickyChanName) then
      begin
        Form1.cbIPTVchan.ItemIndex := i;
        Break;
      end;
  finally
    Form1.cbIPTVchan.Items.EndUpdate;
  end;
end;
